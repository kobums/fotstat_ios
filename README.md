...
| **부상 관리** | 팀 홈 부상자 명단, 부상 등록/수정(부위·발생일·복귀일·메모), 부상 기간 중 기록 입력 차단 |
| **통계** | 날짜 범위 필터, 득점/어시스트/출전시간 순위, 승률, 선수 상세 비교, **부상 결장(결장 N경기)** |
...
└── Features/
    ...
    ├── Record/                  # 선수별 기록 입력 (부상 선수 비활성)
    ├── Injury/                  # 부상자 명단, 등록/수정 폼, 결장 집계
    ├── Stats/                   # 통계, 선수 상세 비교
...
└── TeamContextView (TabView)
    ├── 홈 탭 — 대시보드, 최근 경기, 캘린더, 부상자 명단(등록/수정 시트)
    ├── 선수단 탭 — 포지션별 선수 목록 + 시즌 통계
    ├── 경기 탭 — 예정/완료 경기 목록
    │   └── MatchDetailView — 쿼터 스코어 + 원정 골 수정
    │       └── RecordView — 선수별 기록 입력 (부상 선수 비활성)
    └── 통계 탭 — 날짜 필터 + 순위 + 선수 상세(결장 N경기)
```
